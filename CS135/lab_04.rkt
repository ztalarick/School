...
(define (element? item list-of-items)
  (if (not (null? list-of-items)) (if (equal? item (car list-of-items)) #t (element? item (cdr list-of-items))) #f))
...
(define (intersection listA listB)
  (make-set (if (not (null? listA)) (if (element? (car listA) listB) (append (list (car listA)) (intersection (cdr listA) listB)) (intersection (cdr listA) listB)) '())))
...
(define (subset? setA setB)
  (if (equal? (intersection setA setB) setA) #t #f))
...
(define (set-equal? setA setB)
  (if (or (equal? (intersection setA setB) setA) (equal? (intersection setA setB) setB)) #t #f))
...
(define (set-difference listA listB)
  (make-set (if (equal? listA '()) '() (if (element? (car listA) (intersection listA listB)) (set-difference (cdr listA) listB) (cons (car listA) (set-difference (cdr listA) listB))))))
...
(define (cardinality lst)
  (length (make-set lst)))
  ;(if (equal? lst '()) 0 (+ 1 (cardinality (make-set(cdr lst))))))
...
(define (disjoint? setA setB)
  (if (equal? (intersection setA setB) '()) #t #f))
...
(define (superset? setA setB)
  (equal? (intersection setA setB) setB))
...
(define (remove element set)
  (make-set (set-difference set (list element))))


;; January 2018
;; Samuel Kraus and Edward Minnix
;; Stevens Institute of Technology(list element)
